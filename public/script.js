// ====================Soko exchange TECH - UNIFIED SYSTEM (NO BLINKING) ====================
// Single source of truth - No duplicate functions

// Database Manager
const Soko exchangeDB = {
    init() {
        if (!localStorage.getItem('Soko exchange_users')) {
            localStorage.setItem('Soko exchange_users', JSON.stringify([
                { id: 1, username: 'Admin User', email: 'admin@sokoexchange.com', phone: '254711111111', password: 'admin123', role: 'admin', is_active: true, created_at: new Date().toISOString() },
                { id: 2, username: 'John Buyer', email: 'buyer@sokoexchange.com', phone: '254722222222', password: 'buyer123', role: 'buyer', is_active: true, created_at: new Date().toISOString() },
                { id: 3, username: 'Mary Seller', email: 'seller@sokoexchange.com', phone: '254733333333', password: 'seller123', role: 'seller', is_active: true, created_at: new Date().toISOString() }
            ]));
        }
        if (!localStorage.getItem('Soko exchange_products')) {
            localStorage.setItem('Soko exchange_products', JSON.stringify([
        ...
        if (!localStorage.getItem('Soko exchange_payments')) localStorage.setItem('Soko exchange_payments', '[]');
        if (!localStorage.getItem('Soko exchange_unlocks')) localStorage.setItem('Soko exchange_unlocks', '[]');
        if (!localStorage.getItem('Soko exchange_current_user')) localStorage.setItem('Soko exchange_current_user', 'null');
    },
    getUsers() { return JSON.parse(localStorage.getItem('Soko exchange_users') || '[]'); },
    saveUsers(users) { localStorage.setItem('Soko exchange_users', JSON.stringify(users)); },
    getProducts() { return JSON.parse(localStorage.getItem('Soko exchange_products') || '[]'); },
    saveProducts(products) { localStorage.setItem('Soko exchange_products', JSON.stringify(products)); },
    getPayments() { return JSON.parse(localStorage.getItem('Soko exchange_payments') || '[]'); },
    savePayments(payments) { localStorage.setItem('Soko exchange_payments', JSON.stringify(payments)); },
    getUnlocks() { return JSON.parse(localStorage.getItem('Soko exchange_unlocks') || '[]'); },
    saveUnlocks(unlocks) { localStorage.setItem('Soko exchange_unlocks', JSON.stringify(unlocks)); },
    
    getCurrentUser() {
        const data = localStorage.getItem('Soko exchange_current_user');
        return (data && data !== 'null') ? JSON.parse(data) : null;
    },
    setCurrentUser(user) { localStorage.setItem('Soko exchange_current_user', user ? JSON.stringify(user) : 'null'); },
...
function loginUser(identifier, password) {
    const result = Soko exchangeDB.loginUser(identifier, password);
    if (result.success) { Soko exchangeDB.setCurrentUser(result.user); return result; }
    return result;
}
function registerUser(username, email, phone, password, role) {
    ...
    return Soko exchangeDB.registerUser({ username, email, phone, password, role });
}
function logoutUser() { Soko exchangeDB.setCurrentUser(null); window.location.replace('index.html'); }
...
function loadBuyerProducts() {
    ...
    if (!grid) return;
    const user = Soko exchangeDB.getCurrentUser();
    let products = Soko exchangeDB.getProductsWithSeller().filter(p => p.status === 'active');
    ...
    grid.innerHTML = products.map(p => {
        const unlocked = user ? Soko exchangeDB.hasUnlocked(user.id, p.id) : false;
...
function showBuyerProduct(id) {
    ...
    const products = Soko exchangeDB.getProductsWithSeller();
    const p = products.find(p => p.id === id);
    if (!p) return;
    const unlocked = Soko exchangeDB.hasUnlocked(user.id, id);
    let imgs = []; try { imgs = JSON.parse(p.images || '[]'); } catch(e) {}
...
function payToUnlock(productId) {
    ...
    const result = Soko exchangeDB.unlockContact(user.id, productId, phone);
    if (result.success) {
...
function loadSellerProducts() {
    ...
    const products = Soko exchangeDB.getProducts().filter(p => p.seller_id === user.id);
...
function openEditProduct(id) {
    const p = Soko exchangeDB.getProducts().find(p => p.id === id);
    if (!p) return;
    editProductId = id;
...
function saveEditProduct() {
    ...
    if (Soko exchangeDB.updateProduct(editProductId, updates)) { showToast('Updated!'); closeEditModal(); loadSellerProducts(); }
...
function executeDeleteProduct() { if (deleteProductId && Soko exchangeDB.deleteProduct(deleteProductId)) { showToast('Deleted'); closeDeleteModal(); loadSellerProducts(); } }
...
async function submitSellerProduct(e) {
    ...
    const result = Soko exchangeDB.addProduct({ seller_id: user.id, title, description: desc, category: cat, price, location: loc, images: JSON.stringify(images) });
...
function loadAdminStats() {
    const users = Soko exchangeDB.getUsers(), products = Soko exchangeDB.getProducts(), payments = Soko exchangeDB.getPayments();
...
function loadAdminUsers() { adminUsersCache = Soko exchangeDB.getUsers(); renderAdminUsers(adminUsersCache); }
...
function toggleAdminUser(id, curr) { if (Soko exchangeDB.updateUserStatus(id, !curr)) { loadAdminUsers(); loadAdminStats(); showToast(`User ${!curr ? 'activated' : 'deactivated'}`); } }
function loadAdminProducts() { adminProductsCache = Soko exchangeDB.getProductsWithSeller(); renderAdminProducts(adminProductsCache); }
...
function deleteAdminProduct(id) { if (confirm('Delete product?')) { Soko exchangeDB.deleteProduct(id); loadAdminProducts(); loadAdminStats(); showToast('Deleted'); } }
function loadAdminPayments() {
    const payments = Soko exchangeDB.getPayments();
    const users = Soko exchangeDB.getUsers();
...
function adminLogin() {
    ...
    const user = Soko exchangeDB.getUsers().find(u => (u.email === id || u.phone === id) && u.password === pwd && u.role === 'admin');
    if (!user) { if (err) err.innerText = 'Invalid admin credentials'; return; }
    Soko exchangeDB.setCurrentUser(user);
    document.getElementById('loginOverlay')?.classList.add('hidden');
    document.getElementById('adminPanel').style.display = 'block';
    loadAdminDashboard();
}
function adminLogout() { Soko exchangeDB.setCurrentUser(null); location.reload(); }
